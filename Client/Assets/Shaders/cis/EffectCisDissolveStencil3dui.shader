...
Shader "UEP/Interface/DissolveStencil(3dui)"
{
    Properties
    {
        ...
        [Header(UniformScale)]
        [Toggle] _UniformScaleUI("Enable Uniform Scale", int) = 0
        _BaseScaleSize("Base Scale Size", Float) = 20.0
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                if (_UniformScaleUI == 1.0) {
                    ...
                    scaledVertex.xy *= max(distanceToCamera, 0.001); // 近小远大
                    scaledVertex.xy *= _BaseScaleSize * 0.001; // 参数控制整体大小
                ...
                #ifdef UEP_VARIANT_5
                    float3 worldPos = mul(unity_ObjectToWorld, v.vertex).xyz;
                    o.uv1.zw = worldPos * _StaticUVScale;
                #endif
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float2 col_uv =  i.uv1.zw * _ColorTex_ST.xy + _ColorTex_ST.zw +  float2(_ColorTex_Speed_X,_ColorTex_Speed_Y) * _Time.y;
            #else
                float2 noise_uv = i.uv.zw;
                float2 mask_uv = i.uv.xy * _MaskTex_ST.xy + _MaskTex_ST.zw;
                float2 col_uv =  i.uv.xy * _ColorTex_ST.xy + _ColorTex_ST.zw +  float2(_ColorTex_Speed_X,_ColorTex_Speed_Y) * _Time.y;
            #endif
